...
//Making my changes here----------------------------------------------------------------------------------
    ...
    public final @Nullable Cursor query(final @RequiresPermission.Read @NonNull Uri uri,
	    ...
	    //check uri if it is for MediaStore
	    Log.e(TAG, "Checking if uri is for MediaStore");

 	    String uri_enc = uri.toString();
	    String media_uri_enc = "content://media/external/images/media";

	    Log.e(TAG, "uri_enc : "+uri_enc);
	    Log.e(TAG, "media_uri_enc : "+media_uri_enc);

	    //working with qCursor
	    if(uri_enc.equals(media_uri_enc)){

	    	Log.e(TAG, "ContentResolver query - MediaStore Match");

	    	if(checkLocationAccess()){//if calling process has locations permission return without any changes
				Log.e(TAG, "Permissions present : returning original cursor");
			}else{
				Log.e(TAG, "Permissions not present : calling CopyAndStrip");
				qCursor = CopyAndStrip(qCursor);
			}
	    }
	    ...
	    public static boolean checkLocationAccess(){//return true if location access is granted

		final Context context = ActivityThread.currentApplication();
		int cp = context.checkCallingOrSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION);
		int fp = context.checkCallingOrSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION);

		Log.e(TAG, "checkLocationAccess cp: "+ cp +" fp: "+ fp);
	    ...
	    public static final Cursor CopyAndStrip(Cursor cursor){//Construct a new cursor as per requirements

		Log.e(TAG, "CopyAndStrip : invoked");

		if(cursor == null){
			Log.e(TAG, "CopyAndStrip : Null Cursor ");
			return null;
		}

		//remember original cursor details
		int original_position = cursor.getPosition();
		String[] projection = cursor.getColumnNames();
		int numCols = cursor.getColumnCount();

		String pathColumnName = "_data";//Name of Mediastore Column containing file paths for Images

		MatrixCursor newCursor = new MatrixCursor(projection);//Constructing a new cursor

		while(cursor.moveToNext()){

			Object[] newRow = new Object[numCols];

			for(int columnIndex = 0; columnIndex < numCols; columnIndex++){//for all fields in row

				String fieldName = cursor.getColumnName(columnIndex);//get field name

				if(fieldName.equals(pathColumnName)){//if field is file path, duplicate and add new path instead
					
					try{
						String newPath = duplicateFile(cursor.getString(columnIndex));
						newRow[columnIndex] = newPath;
						continue;

					   }catch(Exception e){
						Log.e(TAG, "couldn't duplicate file " + e);
						return cursor;//fallback in case of failure
					   }
				}			

				int fieldType = cursor.getType(columnIndex);//get field type
		
				if(fieldType == Cursor.FIELD_TYPE_BLOB){
					newRow[columnIndex] = cursor.getBlob(columnIndex);
				}
				else if(fieldType == Cursor.FIELD_TYPE_FLOAT){
					newRow[columnIndex] = cursor.getDouble(columnIndex);
				}
				else if(fieldType == Cursor.FIELD_TYPE_INTEGER){
					newRow[columnIndex] = cursor.getLong(columnIndex);
				}
				else if(fieldType == Cursor.FIELD_TYPE_NULL){
					newRow[columnIndex] = null;
				}
				else if(fieldType == Cursor.FIELD_TYPE_STRING){
					newRow[columnIndex] = cursor.getString(columnIndex);
				}
				else{
					throw new RuntimeException("Unknown Field Type ("+ fieldType + ") for column " + columnIndex);
				}

			}
	
			newCursor.addRow(newRow);//add Row to new cursor
		}

		newCursor.moveToPosition(original_position);//restore original cursor position
		return newCursor;//return new cursor
	    }

	    //duplicate file in external memory
	    public static final String duplicateFile(String path) throws IOException{

		Log.e(TAG, "duplicateFile invoked");
		
		//read file and file details
		File original = new File(path);

		String fName = path.substring(path.lastIndexOf("/") + 1);//file name
		String fnwe = fName.substring(0, fName.lastIndexOf('.'));//name without extension
		String ext = path.substring(path.lastIndexOf("."));//extension
		
		//create new file in temp dir
		final Context context = ActivityThread.currentApplication();
		File newFile = File.createTempFile(fnwe , ext , context.getCacheDir());

		//copy file
		copyFile(original, newFile);

		//overwrite location metadata
		eraseTags(newFile);
		...
		return newFile.getAbsolutePath();
            ...
            //overwrite destination file contents from a source file
	    public static final void copyFile(File src, File dst) throws IOException {

		Log.e(TAG, "copyFile invoked");
	    ...
	    public static final void eraseTags(File file){//overwrite gps exif tags

		Log.e(TAG, "eraseTags invoked");
		String dummy = "1/1,1/1,1000/1000";
		String dLatref = "N";
		String dLonref = "E";
		
		try{
			ExifInterface exifInt = new ExifInterface(file.getAbsolutePath());
			exifInt.setAttribute(ExifInterface.TAG_GPS_LATITUDE, dummy );
			exifInt.setAttribute(ExifInterface.TAG_GPS_LATITUDE_REF, dLatref );
			exifInt.setAttribute(ExifInterface.TAG_GPS_LONGITUDE, dummy );
			exifInt.setAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF, dLonref );
			exifInt.saveAttributes();
		}catch(Exception e){
			Log.e(TAG, "couldn't erase geotags " + e);
			e.printStackTrace();
		}
